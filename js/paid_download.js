...
function drag_drop(event) {
    ...
    ajax.open("GET","../../../za/checking_song/"+file_name, true);
...
function file_check(){
    ...
    var ajax = new XMLHttpRequest();
    ajax.open("GET","../../../za/checking_song_paid/"+file_name, true);
    ajax.setRequestHeader("Content-type","application/x-www-form-urlencoded");
    ajax.onreadystatechange = function () {
        if(ajax.readyState ==  4 && ajax.status == 200){
            if(ajax.responseText !== "ok"){
...
function completeHandler(event) {
    ...
    if(event.target.responseText == 'ok') {
        _("statuss").innerHTML = "";
        _("file").value = "";
        window.location = "../../../za/track_uploaded";
    }else{
        _("error").innerHTML = event.target.responseText;
        _("statuss").innerHTML = "";
        _("file").value = "";
